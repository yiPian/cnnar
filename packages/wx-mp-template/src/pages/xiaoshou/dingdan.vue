<template>
    <ListView :query-func="useDingdanQuery" to-create="xiaoshou/dingdan-add" ref="listView">
            ...
            <div class="bg-white padding-sm margin-sm" v-for="el in data" :key="el.id" @click="toInfo(el.id)">
                ...
                <div class="text-gray">
                    <div class="flex justify-between align-center margin-tb-sm">
                        <span>客户名称</span>
                        <span class="text-black">{{ el.guest_name }}</span>
                    </div>
                    <div class="flex justify-between align-center margin-tb-sm">
                        <span>订货数量</span>
...
<script setup lang="ts">
    import { useDingdanQuery } from '@/api/order'
    import ListView from '@/components/list-view/index.vue'
    const toInfo = (id: number) => {
        uni.navigateTo({ url: '/pages/xiaoshou/dingdan-info?id=' + id })
    }
</script>