<template>
     <div class="margin-sm bg-white radius">
        <van-cell-group>
            <van-cell title="单据类型" value="销售订单" />
    ...
    <div class="padding-sm">
        <van-button :loading="isLoading" loading-text="提交中..." type="primary" block @click="submit">提交订单</van-button>
    </div>
    <SuccessPanel info-path="xiaoshou/dingdan-info" :id="orderId" ref="successPanel"/>
</template>
<script setup lang="ts">
    import { ref } from 'vue'
    import { userDingdanCreate, useShiptypeQuery, useBillmark } from '@/api/order'
    ...
    const submit = () => {
        if(useOrderSubmitVerify(formData.value)) {
            ...
            userDingdanCreate(formData.value).then(res => {
                console.log(res)
                // @ts-ignore
                if (res?.ID) {
                    // @ts-ignore
