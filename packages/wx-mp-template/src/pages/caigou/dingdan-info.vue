    ...
    <template v-if="info">
        <div class="margin-sm bg-white radius" >
            <van-cell-group>
                ...
                <van-cell title="预计到货" :value="info.arrival_date" />
                <van-cell title="供应商名称" :value="info.vender_name" />
                <van-cell title="供应商编码" :value="info.vender_code" />
                <van-cell title="收货仓库" :value="info.arrival_date" />
                <van-cell title="上游单号" :value="info.up_bill_code" />
...
<script setup lang="ts">
    import { ref } from 'vue'
    import { useCgDingdanInfo } from '@/api/order'
    ...
    onLoad((params) => {
        console.log(params)
        if (params && params.id) {
            useCgDingdanInfo(params.id).then(res => {
                // @ts-ignore
                info.value = res
            }).finally(() => {
