<template>
    <div class="margin-sm bg-white radius">
        <van-cell-group>
            <van-cell title="单据类型" value="采购开单" />
    ...
    <SuccessPanel info-path="caigou/kaidan-info" :id="orderId" ref="successPanel"/>
</template>
<script setup lang="ts">
    import { ref } from 'vue'
    import { useBillmark, useCgKaidanCreate } from '@/api/order'
    ...
    const submit = () => {
        if (useOrderSubmitVerify(formData.value, false)) {
            ...
            useCgKaidanCreate(formData.value).then(res => {
                 // @ts-ignore
                 if (res?.ID) {
                    // @ts-ignore
