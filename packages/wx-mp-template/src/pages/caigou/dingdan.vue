<template>
    ...
    <ListView :query-func="useCgDingdanQuery" to-create="caigou/dingdan-add">
        <template v-slot="{ data }">
            <div class="bg-white padding-sm margin-sm" v-for="el in data" :key="el" @click="toInfo(el.id)">
                ...
                <div class="text-gray">
                    ...
                    <div class="flex justify-between align-center margin-tb-sm">
                        <span>预计到货</span>
                        <span class="text-black">{{ el.arrival_date }}</span>
                    </div>
                    <div class="flex justify-between align-center margin-tb-sm">
                        <span>单据日期</span>
...
<script setup lang="ts">
    import { useCgDingdanQuery } from '@/api/order'
    import ListView from '@/components/list-view/index.vue'
    const toInfo = (id:number) => {
        uni.navigateTo({ url: '/pages/caigou/dingdan-info?id=' + id })
    }
</script>